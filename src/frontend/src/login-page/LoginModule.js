...
const changePassword = async (changeState, username, password1, password2) => {
  if (lib.validatePassword(changeState, password1, password2, '/loginchangepassword/invalid')) {
    const toUserId = await database.sendGetRequest(`http://localhost:8080/user-by-name/${username}`);
    if (toUserId.length === 0) {
      changeState({ link: '/loginchangepassword/user' });
      return;
    }

    await database.sendPutRequest(`http://localhost:8080/user/${toUserId[0].user_id}`, { user_password: createHash('sha256').update(password1).digest('hex') });
