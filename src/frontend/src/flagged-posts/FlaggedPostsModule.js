...
const getFlaggedPostsForReview = async (state) => {
  const { flaggedPosts } = await database.sendGetRequest(`http://localhost:8080/flag-post/${state.userId}`);
  return flaggedPosts;
};
...
const finishFlag = async (flag, deleted, changeState) => {
  ...
  await database.sendBodiedDeleteRequest(`http://localhost:8080/flag-post/${flag.post_id}`, body);
