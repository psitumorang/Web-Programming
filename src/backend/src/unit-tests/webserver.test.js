...
const deleteUser = async (name) => {
  await knex('user_lst').where('user_name', name).del();
};
...
const deleteGroupMember = async (groupId, id) => {
  await knex('group_members').where('member_id', id).del();
};

const deleteMessages = async (convoId) => {
...
const deleteAdmins = async (groupId, id) => {
  await knex('admin_lst').where('group_id', groupId).del();
};

const deletePostFlags = async (postId) => {
...
const deleteGroupTopics = async (groupId) => {
...
describe('Test webapp endpoints and integration tests', () => {
	...
	test('status code and response default response', async () => await request(webapp).get('/doesntexist')
	  .expect(404));
  ...
  test('status code and response get and delete profile', async () => {
	...
	test('status code and response creating text post', async () => {
  	...
  	const response2 = await request(webapp).post('/groups')
  		.send({group_name: 'test', group_creator: userId, group_description: 'its a group', is_public: 1});
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		const response3 = await request(webapp).post('/post/text')
			.send({post_group: groupId, posting_user: userId, caption: 'its a post', posting_username: 'mcleesm'});
		...
		await deletePost(ret3.post.post_id);
		await deleteGroupMember(groupId, userId);
		await deleteAdmins(groupId, userId);
	...
	test('status code and response creating video post', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		const response3 = await request(webapp).post('/post/video')
			.send({post_group: groupId, posting_user: userId, caption: 'its a post', videoUrl: 'wow it has video', posting_username: 'mcleesm'});
		expect(response3.status).toBe(201);
		const ret3 = JSON.parse(response3.text);
		//get the post from the user id
		const response4 = await request(webapp).get(`/post/${userId}`)
	...
	test('status code and response creating audio post', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		const response3 = await request(webapp).post('/post/audio')
			.send({post_group: groupId, posting_user: userId, caption: 'its a post', audioUrl: 'wow it has audio', posting_username: 'mcleesm'});
		expect(response3.status).toBe(201);
		const ret3 = JSON.parse(response3.text);
		//get the post from the user id
		const response4 = await request(webapp).get(`/post/${userId}`)
	...
	test('status code and response creating image post', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		const response3 = await request(webapp).post('/post/image')
			.send({post_group: groupId, posting_user: userId, caption: 'its a post', photourl: 'wow it has photos', posting_username: 'mcleesm'});
		expect(response3.status).toBe(201);
		const ret3 = JSON.parse(response3.text);
		//get the post from the user id
		const response4 = await request(webapp).get(`/post/${userId}`)
	...
	test('status code and response group topics', async () => {
  	...
  	const userId = ret.user.user_id;
  	const response2 = await request(webapp).post('/groups')
  		.send({group_name: 'test', group_creator: userId, group_description: 'its a group', is_public: 1, topic_1: 'topic1', topic_2: 'topic2', topic_3: 'topic3'});
  	expect(response2.status).toBe(201);
  	const ret2 = JSON.parse(response2.text)
		const groupId = ret2.group.group_id;
		...
		await deleteGroupTopics(groupId);
		await deleteGroupMember(groupId, userId);
		await deleteAdmins(groupId, userId);
		await deleteGroup(groupId);
		await deleteProfile(userId);
	...
	test('status code and response creating text messages and convos', async () => {
  	...
  	//create group 
		const response3 = await request(webapp).post('/groups')
  		.send({group_name: 'test', group_creator: userId1, group_description: 'its a group', is_public: 1});
  	expect(response3.status).toBe(201);
  	const ret3 = JSON.parse(response3.text)
  	...
  	const response4 = await request(webapp).post('/admins')
  		.send({admin: {groupId: groupId, isCreator: 0, adminUser: 'mcleesm1'}});
  	expect(response4.status).toBe(201);

  	//create conversation
  	//send a text message
  	const response5 = await request(webapp).post(`/message/text/${userId2}`)
  		.send({ msg: { toId: userId2, fromId: userId1, senderName: 'mcleesm', receiverName: 'mcleesm1', txt: 'hi bro'} });
  	...
  	const convoId = JSON.parse(response6.text)[0].convoId;
  	console.log(convoId);
  	//send a video message
		const response8 = await request(webapp).post(`/message/video/${userId2}`)
  		.send({ msg: { toId: userId2, fromId: userId1, senderName: 'mcleesm', receiverName: 'mcleesm1', video: 'omg a url'} });
  	expect(response8.status).toBe(201);

  	//send an audio message
  	const response9 = await request(webapp).post(`/message/audio/${userId2}`)
  		.send({ msg: { toId: userId2, fromId: userId1, senderName: 'mcleesm', receiverName: 'mcleesm1', audio: 'omg a url'} });
  	expect(response9.status).toBe(201);

  	//send an image message
  	const response10 = await request(webapp).post(`/message/image/${userId2}`)
  		.send({ msg: { toId: userId2, fromId: userId1, senderName: 'mcleesm', receiverName: 'mcleesm1', img: 'omg a url'} });
  	expect(response10.status).toBe(201);

  	await deleteMessages(convoId);
  	await deleteConvo(convoId);
  	await deleteGroupMember(groupId, userId1);
  	await deleteGroupMember(groupId, userId2);
  	await deleteAdmins(groupId, userId1);
  	await deleteAdmins(groupId, userId2);
		await deleteGroup(groupId);
		await deleteProfile(userId1);
		await deleteProfile(userId2);
		await deleteUser('mcleesm');
		await deleteUser('mcleesm1');
	});
	...
	test('create and flag a post, post gets deleted', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		//create a post
		const response3 = await request(webapp).post('/post/image')
			.send({post_group: groupId, posting_user: userId, caption: 'its a post', photourl: 'wow it has photos', posting_username: 'mcleesm5'});
		expect(response3.status).toBe(201);
		const ret3 = JSON.parse(response3.text);
		...
		const response5 = await request(webapp).put(`/flag-post/${postId}`)
		  .send({flaggerId: userId, flaggerName: 'mcleesm5', groupId: groupId, groupName: 'test'});
		expect(response5.status).toBe(201);
		...
		const response7 = await request(webapp).delete(`/flag-post/${postId}`)
			.send({flaggerId: userId, flaggerName: 'mcleesm5', groupId: groupId, deleted: 1, author: 'mcleesm5'});
		expect(response7.status).toBe(200);

		await deleteGroupMember(groupId, userId);
		await deletePostFlags(postId);
		await deleteAdmins(groupId, userId);
		await deleteGroup(groupId);
		await deleteProfile(userId);
		await deleteUser('mcleesm5');
	...
	test('create and flag a post, post doesnt get deleted', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		//delete flagged post
		const response7 = await request(webapp).delete(`/flag-post/${postId}`)
			.send({flaggerId: userId, flaggerName: 'mcleesm5', groupId: groupId, deleted: 0, author: 'mcleesm5'});
		expect(response7.status).toBe(200);
	...
	test('add comments to post, then hide them', async () => {
  	...
  	const ret2 = JSON.parse(response2.text)
		...
		const response5 = await request(webapp).post('/reply')
		  .send({ post_id: postId, post_group: groupId, posting_user: userId, caption: 'woah' });
		expect(response5.status).toBe(201);
