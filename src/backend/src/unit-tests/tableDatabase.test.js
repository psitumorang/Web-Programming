...
const clearDbAdmin = async (id) => {
	await knex('admin_lst').where('admin_id', id).del();
};

const clearDbConvo = async (id) => {
	await knex('convo_lst').where('convoId', id).del();
};

const clearDbInvite = async (id) => {
	await knex('invitations').where('invitation_id', id).del();
};
...
const clearDbNotif = async (id) => {
	await knex('notification_lst').where('user_id', id).del();
};

const clearDbPost = async (id) => {
	await knex('post_lst').where('post_id', id).del();
};

const clearDbGroup = async (id) => {
	await knex('group_lst').where('group_id', id).del();
	await knex('user_lst').where('user_id', id).del();
};

const clearDbProfile = async (id) => {
	await knex('profile_lst').where('user_id', id).del();
};

const clearDbReply = async (id) => {
	await knex('reply_lst').where('post_id', id).del();
};

const clearDbUser = async (id) => {
	await knex('user_lst').where('user_id', id).del();
};
...
describe('admin database operations tests', () => {
	...
	test('test get all admins successful', async () => {
		...
		expect()
		await clearDbAdmin(-2);
		await clearDbAdmin(-3);
		await clearDbAdmin(-1);
	});
	...
	test('get group memberships by user id', async () => {
		...
		await knex('group_lst').insert({group_id: -4, group_name: 'group', group_creator: -5, group_description: 'description', is_public: 1});
		await knex('group_lst').insert({group_id: -5, group_name: 'group', group_creator: -5, group_description: 'description', is_public: 1});
	...
	test('delete membership by user id', async () => {
		...
		await knex('group_lst').insert({group_id: -3, group_name: 'group', group_creator: -4, group_description: 'description', is_public: 1});
	...
	test('delete memberships by group id and user id', async () => {
		...
		await knex('group_lst').insert({group_id: -6, group_name: 'group', group_creator: -6, group_description: 'description', is_public: 1});
	...
	test('get groups for user', async () => {
		...
		await knex('group_lst').insert({group_id: -7, group_name: 'group', group_creator: -7, group_description: 'description', is_public: 1});
		await knex('group_lst').insert({group_id: -8, group_name: 'group', group_creator: -7, group_description: 'description', is_public: 1});
	...
	test('add invitation', async () => {
		const invite = await inviteLib.addInvitation(db, {toUserId: -1, fromUserId: -2, invitationStatus: 'pending', groupId: -1});
		console.log(invite);
		...
		await clearDbInvite(invite.insertId);
	});

	test('get pending invitations', async () => {
		const invite = await inviteLib.addInvitation(db, {toUserId: -1, fromUserId: -2, invitationStatus: 'pending', groupId: -1});
		const pending = await inviteLib.getPendingInvitations(db, -1);
		console.log(pending);

		expect(pending[0].invitation_id).toBe(invite.insertId);
		expect(pending[0].to_user_id).toBe(-1);
		expect(pending[0].from_user_id).toBe(-2);
		expect(pending[0].invitation_status).toBe('pending');
		expect(pending[0].group_id).toBe(-1);
		await clearDbInvite(invite.insertId);
	});
	...
	test('delete pending invites', async () => {
		...
		const pending = await inviteLib.deletePendingInvites(db, -1);
		const ret = await knex.select('*').from('invitations').where('invitation_id', invite.insertId);
	...
	test('get open invites by group id', async () => {
		...
		const pending = await inviteLib.getOpenInvitesByGroupId(db, -1);
		expect(pending[0].invitation_id).toBe(invite.insertId);
		expect(pending[0].to_user_id).toBe(-1);
		expect(pending[0].from_user_id).toBe(-2);
		expect(pending[0].invitation_status).toBe('pending');
	...
	test('add group no error', async () => {
		...
		const group = await groupLib.addGroup(db, {group_id: -10, group_name: 'name', group_creator: -10, group_description: 'description', is_public: 1});
		...
		await clearDbGroup(-10);
	});

	test('add group error', async () => {
		await knex('user_lst').insert({user_id: -11, user_name: 'me', user_password: 'password'});
		const group = await groupLib.addGroup(db, {group_id: -11, group_name: 'name', group_creator: -11, group_description: 'description', is_public: 1});
		const group2 = await groupLib.addGroup(db, {group_id: -12, group_name: 'name', group_creator: -11, group_description: 'description', is_public: 1});

		expect(group2).toBe(null);
		await knex('group_lst').where('group_id', -12).del();
		await clearDbGroup(-11);
	});

	test('add topics for group', async () => {
		await knex('user_lst').insert({user_id: -12, user_name: 'me', user_password: 'password'});
		const group = await groupLib.addGroup(db, {group_id: -13, group_name: 'name1', group_creator: -12, group_description: 'description', is_public: 1});
		const topics = await groupLib.addTopics(db, {group_id: -13, topic_1:'topic1', topic_2:'topic2', topic_3:'topic3'});
	...
	test('get all topics', async () => {
		await knex('user_lst').insert({user_id: -12, user_name: 'me', user_password: 'password'});
		const group = await groupLib.addGroup(db, {group_id: -13, group_name: 'name1', group_creator: -12, group_description: 'description', is_public: 1});
		await groupLib.addTopics(db, {group_id: -13, topic_1:'topic1', topic_2:'topic2', topic_3:'topic3'});
	...
	test('get topics for group', async () => {
		...
		const group = await groupLib.addGroup(db, {group_id: -14, group_name: 'name2', group_creator: -14, group_description: 'description', is_public: 1});
		await groupLib.addTopics(db, {group_id: -14, topic_1:'topic1', topic_2:'topic2', topic_3:'topic3'});
	...
	test('get groups with topic', async () => {
		...
		const group = await groupLib.addGroup(db, {group_id: -15, group_name: 'name2', group_creator: -15, group_description: 'description', is_public: 1});
		const group2 = await groupLib.addGroup(db, {group_id: -16, group_name: 'name3', group_creator: -15, group_description: 'description', is_public: 1});
	...
	test('get public groups (sorting)', async () => {
		...
		await knex('group_lst').insert({group_id: -17, group_name: 'group1', group_creator: -16, group_description: 'description', is_public: 1, member_number: 4, post_number: 100});
		await knex('group_lst').insert({group_id: -18, group_name: 'group2', group_creator: -16, group_description: 'description', is_public: 1, member_number: 60, post_number: 0});
	...
	test('get groups', async () => {
		...
		const group = await groupLib.addGroup(db, {group_id: -19, group_name: 'name2', group_creator: -19, group_description: 'description', is_public: 1});
		const group2 = await groupLib.addGroup(db, {group_id: -20, group_name: 'name3', group_creator: -19, group_description: 'description', is_public: 1});
	...
	test('get groups by id', async () => {
		...
		const group2 = await groupLib.addGroup(db, {group_id: -21, group_name: 'name3', group_creator: -20, group_description: 'description', is_public: 1});
	...
	test('get groups by with name', async () => {
		...
		const group2 = await groupLib.addGroup(db, {group_id: -22, group_name: 'name3', group_creator: -21, group_description: 'description', is_public: 1});
	...
	test('delete group', async () => {
		...
		const group2 = await groupLib.addGroup(db, {group_id: -23, group_name: 'name3', group_creator: -22, group_description: 'description', is_public: 1});
	...
	test('update group', async () => {
		...
		const group2 = await groupLib.addGroup(db, {group_id: -24, group_name: 'name3', group_creator: -23, group_description: 'description', is_public: 1});
	...
	test('add text post', async () => {
		...
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		const post = await postLib.addTextPost(db, {post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', posting_username: 'me'});
	...
	test('add image post', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		const post = await postLib.addImagePost(db, {post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', photourl: 'url', posting_username: 'me'});
		const ret = await knex.select('*').from('post_lst').where('post_id', -40);
	...
	test('add audio post', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		const post = await postLib.addAudioPost(db, {post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', audioUrl: 'url', posting_username: 'me'});
		const ret = await knex.select('*').from('post_lst').where('post_id', -40);
	...
	test('add video post', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		const post = await postLib.addVideoPost(db, {post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', videoUrl: 'url', posting_username: 'me'});
		const ret = await knex.select('*').from('post_lst').where('post_id', -40);
	...
	test('get user posts', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		await postLib.addVideoPost(db, {post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', videoUrl: 'url', posting_username: 'me'});
	...
	test('add profile', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		const profile = await profileLib.addProfile(db, {user_id: -40, first_name: 'your', last_name: 'mom', biography: 'hi', profile_picture_url: 'url'});
	...
	test('add reply to post', async () => {
		await knex('user_lst').insert({user_id: -40, user_name: 'me', user_password: 'password'});
		await knex('group_lst').insert({group_id: -40, group_name: 'group', group_creator: -40, group_description: 'description', is_public: 1});
		await knex('post_lst').insert({post_id: -40, post_group: -40, posting_user: -40, caption: 'yup', videoUrl: 'url', posting_username: 'me'});
		const rep = await replyLib.addReply(db, {reply_id: -40, post_id: -40, post_group: -40, posting_user: -40, caption: 'yo'});
	...
	test('add user success', async () => {
		...
		await clearDbUser(-50);
	});

	test('add user already exists', async () => {
		const user = await userLib.addUser(db, {user_id: -40, user_name: 'yo', user_password: 'secret'});
		const user2 = await userLib.addUser(db, {user_id: -50, user_name: 'yo', user_password: 'secret'});
		const ret = await knex.select('*').from('user_lst').where('user_id', -40);
